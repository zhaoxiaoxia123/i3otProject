...
@Component({
  selector: 'app-procurement-order',
...
export class ProcurementOrderComponent implements OnInit {
  ...
  getPurchaseList(number:string) {
    let url = this.globalService.getDomain()+'/api/v1/getPurchaseList?type=3&page='+number+'&sid='+this.cookiestore.getCookie('sid');
  ...
  editPurchase(){
    ...
    this.router.navigate(['/procurement-management/app-order/'+id]);
  }
