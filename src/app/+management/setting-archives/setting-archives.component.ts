...
export class SettingArchivesComponent implements OnInit {
    ...
    rollback_url : string = '/product-management/unit-list';
    constructor(
        private http:Http,
     ...
     */
    getProductDefault(){
        this.http.get(this.globalService.getDomain()+'/api/v1/getProductDefault?role='+this.role+'&category_type='+this.category_type+'&sid='+this.cookieStoreService.getCookie('sid'))
            .map((res)=>res.json())
            .subscribe((data)=>{
    ...
    getProductList(number:string) {
        let url = this.globalService.getDomain()+'/api/v1/getProductList?role='+this.role+'&page='+number+'&sid='+this.cookieStoreService.getCookie('sid');
    ...
    getProductInfo(){
        ...
        this.http.get(this.globalService.getDomain()+'/api/v1/getProductInfo?c_id='+c_id)
            .map((res)=>res.json())
            .subscribe((data)=>{
        ...
        setTimeout(() => {
            ...
            if(this.productInfo['result']['c_follow_user_id'] != 0){
                this.getDepartment(this.productInfo['result']['c_follow_user_id'],2);
            }
        }, 500);
    }
    ...
    editProduct(){
        ...
        this.lgModal.show();
        this.http.get(this.globalService.getDomain()+'/api/v1/getProductInfo?c_id='+c_id+'&role='+this.role)
            .map((res)=>res.json())
            .subscribe((data)=>{
                this.productInfo = data;
            });
    ...
    deleteProduct(){
        ...
        msg = '您确定要删除该信息吗？';
        if(confirm(msg)) {
            let url = this.globalService.getDomain()+'/api/v1/deleteProductById?ids=' + ids + '&role='+this.role+'&type=all&sid=' + this.cookieStoreService.getCookie('sid');
