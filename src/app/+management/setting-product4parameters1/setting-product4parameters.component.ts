...
export class SettingProduct4parametersComponent implements OnInit {
  ...
  constructor(
    ...
    categoryChange(value:any,category_desc:string){
        if( ! this.cookieStore.in_array(value,this.join_)){
            this.join_.push(value);
            this.show_join.push(category_desc);
        }else{
            for (let s = 0; s < this.join_.length; s++) {
                if (this.join_[s] ==value) {
                    this.join_.splice(s,1);
                    this.show_join.splice(s,1);
                }
            }
        }
    }
    ...
    deleteJoin($event:Event,sc:any) {
        ...
        for (let s = 0; s < this.show_join.length; s++) {
            if (this.show_join[s] ==sc) {
                this.join_.splice(s,1);
                this.show_join.splice(s,1);
            }
        }
